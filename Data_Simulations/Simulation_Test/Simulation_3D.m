...
niftiwrite(petPhantom, [outputPath sprintf('PhantomPet')]);
...
petGroundTruth = imgaussfilt(petGroundTruth, 2);
% Project and introduce noise
m_sinruido = PET.P(petGroundTruth); % 
...
for k = 1 : numIteraciones
    ...
    x = x./(sensImage+1e-5).*PET.PT(m./(PET.P(x)+1e-5));
    % Compute metrics:
...
niftiwrite(x,[outputPath sprintf('Reconstructed_Image')])
